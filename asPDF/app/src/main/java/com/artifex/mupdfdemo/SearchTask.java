...
public abstract class SearchTask {
	...
	public void go(final String text, int direction, int displayPage, int searchPage) {
		...
		progressDialog.setTitle("earching");
		...
		mSearchTask = new AsyncTask<Void,Integer,SearchTaskResult>() {
			...
			protected void onPostExecute(SearchTaskResult result) {
				...
				if (result != null) {
					...
					mAlertBuilder.setTitle(SearchTaskResult.get() == null ? "Text not found" : "No further occurrences found");
					AlertDialog alert = mAlertBuilder.create();
					alert.setButton(AlertDialog.BUTTON_POSITIVE, "Dimiss",
